#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# PostgreSQL backup — dumps the Kratos database to a compressed file.
#
# Usage:
...
BACKUP_DIR="${1:-${BACKUP_DIR:-/opt/apps/auth-cloak/backups}}"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
BACKUP_FILE="${BACKUP_DIR}/kratos_${TIMESTAMP}.dump.gz"
...
find "$BACKUP_DIR" -name "kratos_*.dump.gz" -mtime "+${RETENTION_DAYS}" -delete -print
